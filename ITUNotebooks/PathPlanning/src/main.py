import numpy as np
from planner import CoveragePlanner, HeuristicType, PlannerStatus
...
def load_map(map_name):
    with open("ITUNotebooks/PathPlanning/maps/{}.npy".format(map_name), "rb") as f:
        return np.load(f)
...
def plot_map(target_map, trajectory, map_name="map", params_str=""):
    ...
    target_map_ref[trajectory[-1][1]][trajectory[-1][2]] = status_to_cmap_pos[
        trajectory[-1][6]
    ]
    ...
    ax.arrow(
        trajectory[0][2] - init_direction[1] / 2,
        trajectory[0][1] - init_direction[0] / 2,
        init_direction[1],
        init_direction[0],
        width=0.1,
    ...
    legend_elements = [
        Line2D(
            [0],
            [0],
            color=status_color_ref[PlannerStatus.COVERAGE_SEARCH],
            lw=1,
            marker=">",
            markerfacecolor=status_color_ref[PlannerStatus.COVERAGE_SEARCH],
            label="Coverage Search",
        ),
        Line2D(
            [0],
            [0],
            color=status_color_ref[PlannerStatus.NEARST_UNVISITED_SEARCH],
            lw=1,
            marker=">",
            markerfacecolor=status_color_ref[PlannerStatus.NEARST_UNVISITED_SEARCH],
            label="A*u Search",
        ),
        Line2D(
            [0],
            ...
            lw=1,
            marker=">",
            markerfacecolor=start_orientation_color,
        ...
        Line2D(
            ...
            color="w",
            label="Start Pos",
            markerfacecolor=start_position_color,
            markersize=15,
        ),
        Line2D(
            [0],
            ...
            color="w",
            label="End Pos",
            markerfacecolor=status_color_ref[trajectory[-1][6]],
            markersize=15,
        ),
        Line2D(
            [0],
    ...
    ax.legend(handles=legend_elements, bbox_to_anchor=(1.025, 1.0), loc="upper left")
    plt.title("Coverage Path Planning - {}\n{}".format(map_name, params_str))
    plt.tight_layout()
    plt.show()
    fig.savefig(
        "ITUNotebooks/PathPlanning/output/{}.png".format(map_name),
        bbox_inches="tight",
        dpi=600,
    )
...
if __name__ == "__main__":
    ...
    for map_name in maps:
        ...
        summary_tb = tabulate(
            summary, compare_tb_headers, tablefmt="pretty", floatfmt=".2f"
        )
        print(summary_tb)
        ...
        plot_map(
            ...
            compare_tb[0][5],
            map_name=map_name,
            params_str="Heuristic:{}, Initial Orientation: {}".format(
                compare_tb[0][0], cp.movement_name[compare_tb[0][1]]
            ),
        )
        ...
        print(
            "\nList of coordinates of the best path: [map:{}, initial orientation: {} ({}), coverage path Heuristic:{}]".format(
                map_name,
                cp.movement_name[compare_tb[0][1]],
                compare_tb[0][1],
                compare_tb[0][0],
            )
        )
